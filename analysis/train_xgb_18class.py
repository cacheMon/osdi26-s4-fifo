#!/usr/bin/env python3
"""
Simple XGBoost Multi-Class Classification for 18 Selected Configs

Uses Greedy Set Cover results to classify into 18 config classes that cover 99.1% of traces.
"""
import os
import pandas as pd
import numpy as np
...
def engineer_features_post_merge(df):
    """Features that depend on 'ratio'"""
    # thrashing_risk: rho_unique / (ratio * 100)
    # We use ratio which is 0.001, 0.01, 0.1
    df['thrashing_risk'] = df['rho_unique'] / (df['ratio'] * 100 + 1e-6)
    
    # scan_intensity: rho_onehit * (1 - ratio)
    df['scan_intensity'] = df['rho_onehit'] * (1.0 - df['ratio'])
...
def main():
    ...
    script_dir = os.path.dirname(os.path.abspath(__file__))
    models_output_dir = os.path.join(script_dir, 'xgb_18class_results')
