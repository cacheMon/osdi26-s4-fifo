#!/usr/bin/env python3
"""
XGBoost Multi-Class Classification for 18 Selected Configs

WSS-ESTIMATED RATIO version (ratio-agnostic):
- Estimates ratio from: ratio_est = cache_size / working_set_size
- working_set_size ≈ total_reqs × rho_unique  
- cache_size = exp(log_C)
- Model generalizes across all cache sizes without explicit ratio input
"""
import os
import pandas as pd
import numpy as np
...
def engineer_features_post_merge(df):
    """Features using WSS-estimated ratio (ratio-agnostic).
    
    Estimates ratio from raw features:
    - working_set_size ≈ total_reqs × rho_unique
    - cache_size = exp(log_C)
    - ratio_estimate = cache_size / working_set_size
    
    Then applies original formulas with estimated ratio.
    """
    import numpy as np
    
    # Estimate ratio from raw features
    cache_size = np.exp(df['log_C'])
    working_set_size = df['total_reqs'] * df['rho_unique']
    ratio_estimate = cache_size / (working_set_size + 1e-6)
    ratio_estimate = ratio_estimate.clip(0.0001, 1.0)
    
    # Apply original feature formulas with estimated ratio
    df['thrashing_risk'] = df['rho_unique'] / (ratio_estimate * 100 + 1e-6)
    df['scan_intensity'] = df['rho_onehit'] * (1.0 - ratio_estimate)
    df['ratio_estimate'] = ratio_estimate
...
def main():
    ...
    keep_features = [
        # Advanced (User Requested)
        'probation_efficiency', 'ghost_pressure', 'entropy_gap', 
        'decay_rate_small', 'tail_heaviness', 
        'thrashing_risk', 'scan_intensity',
        
        # Raw Basic
        'rho_unique', 'rho_onehit', 'total_reqs',
    ...
    feature_cols = list(set(feature_cols)) # Deduplicate
    ...
    script_dir = os.path.dirname(os.path.abspath(__file__))
    models_output_dir = os.path.join(script_dir, 'xgb_18class_results_wss')
    ...
    # Save
    out_dir = 'xgb_18class_results_wss'
    ...
    # Save results
    output_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'xgb_18class_results_wss')
